...
const AffiliateLogin: React.FC = () => {
  ...
  // ── Affiliate state ───────────────────────────────────────────────────────
  const [affiliateView, setAffiliateView] = useState<AffiliateView>(
    location.pathname.includes('/apply') ? 'register' : 'login'
  );
  ...
  const { affiliateLogin, register } = useAuth();
            ...
            <div className="relative z-10 my-8 lg:my-auto py-4 space-y-5">
              <div>
                ...
                <p className="text-xs sm:text-sm text-white/80 leading-relaxed font-semibold mt-2 max-w-md">
                  Join our affiliate partner program to earn commissions on every successful WAEC result-checker voucher sale generated through your referrals.
                </p>
              </div>
            </div>
            ...
            <div className="max-w-md w-full mx-auto my-auto py-2 space-y-5">
              ...
              <div>
                <h1 className={`text-2xl sm:text-3xl font-black tracking-tight ${isLight ? 'text-slate-950' : 'text-white'}`}>
                  {affiliateView === 'register' ? 'Become an Affiliate' : 'Affiliate Sign In'}
                </h1>
              ...
              <div className="mt-4 pt-3 border-t border-slate-200 dark:border-slate-800 text-center">
                  <Link to="/admin/login" className={`text-xs font-black hover:underline ${isLight ? 'text-slate-500 hover:text-slate-950' : 'text-slate-400 hover:text-white'}`}>
