...
const AffiliateRegister: React.FC = () => {
  ...
  const handleNextStep = () => {
    setSubmitError('');
    if (registerStep === 1) {
      if (!registerForm.fullName || !registerForm.phone) {
        setSubmitError('Please provide your name and phone number.');
        return;
      }
      setRegisterStep(2);
    } else if (registerStep === 2) {
      if (!registerForm.email || !registerForm.location) {
        setSubmitError('Please provide your email and location.');
        return;
      }
  ...
  const handleAffiliateRegister = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      setSubmitSuccess('Application submitted successfully! Welcome to the Resulta Affiliate program.');
      setTimeout(() => navigate('/affiliate/dashboard', { replace: true }), 1500);
    } else {
      setSubmitError(result.error || 'Application failed. Please try again or contact support.');
    }
  };
              ...
              <div className="mb-8">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className={`text-xs font-bold ${isLight ? 'text-slate-400' : 'text-slate-500'}`}>
                    ...
                    {registerStep === 2 && 'Account Details'}
              ...
              <form onSubmit={handleAffiliateRegister} className="space-y-5" noValidate>
                ...
                {registerStep === 1 && (
                  <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
                    ...
                    />
                    <InputField id="reg-phone" label="Phone Number (Mobile Money)" type="tel" value={registerForm.phone} icon={<FiPhone />} placeholder="024XXXXXXX" isLight={isLight}
                    ...
                    <div className={`p-4 rounded-xl border ${isLight ? 'bg-slate-50 border-slate-200' : 'bg-slate-900/50 border-slate-800'}`}>
                      <label className="flex items-start gap-3 cursor-pointer">
                        <input
                          ...
                          className="mt-1 w-4 h-4 rounded border-slate-300 text-teal-600 focus:ring-teal-500 cursor-pointer transition-colors"
                        />
