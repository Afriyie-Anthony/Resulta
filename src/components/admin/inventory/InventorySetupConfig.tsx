...
import { FiSettings, FiSave, FiBell, FiDollarSign, FiMail, FiSmartphone } from 'react-icons/fi';
...
export const InventorySetupConfig: React.FC<InventorySetupConfigProps> = ({ stats, onUpdateThresholds }) => {
  ...
  const [alertEmail, setAlertEmail] = useState('admin@resulta.gh, devops@resulta.gh');
  const [alertPhone, setAlertPhone] = useState('+233 24 555 0192');
    ...
    <form onSubmit={handleSaveConfig} className="space-y-6 max-w-4xl mx-auto">
      <div className={`p-8 rounded-3xl border transition-colors shadow-sm ${
        ...
        <div className="flex items-center gap-3 border-b pb-4 mb-6 border-slate-200 dark:border-slate-800">
          ...
          <div>
            ...
            <p className={`text-xs font-medium ${isLight ? 'text-slate-500' : 'text-slate-400'}`}>
              Adjust automated alerting parameters, retail unit prices, and notification distribution channels
            </p>
          </div>
        </div>
        ...
        <div className="space-y-8">
          ...
          <div>
            <h4 className={`text-sm font-black flex items-center gap-2 mb-4 ${isLight ? 'text-slate-800' : 'text-slate-200'}`}>
              <FiBell className={isLight ? 'text-secondary' : 'text-teal-400'} /> Low Stock Trigger Thresholds
            </h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                ...
                <div className="relative">
                  ...
                  />
                  <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-slate-400 font-bold">PIN Units</span>
                </div>
                <p className="text-[11px] text-slate-400 mt-1">Triggers emergency alerts when WASSCE count falls below this number.</p>
              </div>
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400 mt-1">Triggers emergency alerts when BECE count falls below this number.</p>
              </div>
            </div>
          </div>
          ...
          <div className="pt-6 border-t border-slate-200 dark:border-slate-800">
            <h4 className={`text-sm font-black flex items-center gap-2 mb-4 ${isLight ? 'text-slate-800' : 'text-slate-200'}`}>
              <FiDollarSign className={isLight ? 'text-secondary' : 'text-teal-400'} /> Retail Unit Checkout Prices (GH₵)
            </h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                <label className={`block text-xs font-bold mb-2 ${isLight ? 'text-slate-700' : 'text-slate-300'}`}>
          ...
          {/* Section 3: Notification Distribution */}
          <div className="pt-6 border-t border-slate-200 dark:border-slate-800">
            <h4 className={`text-sm font-black flex items-center gap-2 mb-4 ${isLight ? 'text-slate-800' : 'text-slate-200'}`}>
              <FiMail className={isLight ? 'text-secondary' : 'text-teal-400'} /> Alert Distribution Channels
            </h4>
            <div className="space-y-4">
              <div>
                <label className={`block text-xs font-bold mb-1.5 ${isLight ? 'text-slate-700' : 'text-slate-300'}`}>
                  Automated Email Receivers (comma separated)
                </label>
                <input
                  type="text"
                  value={alertEmail}
                  onChange={(e) => setAlertEmail(e.target.value)}
                  className={`w-full rounded-xl px-4 py-2 text-xs font-semibold focus:outline-none transition-colors border ${
                    isLight
                      ? 'bg-slate-50 border-slate-200 text-slate-900 focus:border-secondary focus:bg-white'
                      : 'bg-slate-950 border-slate-800 text-white focus:border-teal-500'
                  }`}
              ...
              <div>
                <label className={`block text-xs font-bold mb-1.5 flex items-center gap-1 ${isLight ? 'text-slate-700' : 'text-slate-300'}`}>
                  <FiSmartphone /> SMS Broadcast Phone Numbers
                </label>
                <input
                  type="text"
                  value={alertPhone}
        ...
        <div className={`flex justify-end gap-3 pt-8 mt-8 border-t ${isLight ? 'border-slate-200' : 'border-slate-800'}`}>
