...
import { FiSearch, FiPhoneCall, FiShield, FiCheckCircle } from 'react-icons/fi';
...
export const CustomerLayout: React.FC<BaseComponentProps> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 text-slate-800 relative selection:bg-teal-600 selection:text-white">
      {/* Background Ambient Glow */}
      <div className="fixed inset-0 pointer-events-none z-0 bg-radial-glow" />

      {/* Top Banner Notice */}
      <div className="bg-gradient-to-r from-teal-800 via-teal-700 to-emerald-800 text-teal-50 text-xs py-2 px-4 border-b border-teal-600/20 text-center relative z-20 flex items-center justify-center gap-2 shadow-sm">
        <span className="bg-white/20 text-white font-bold px-2 py-0.5 rounded text-[10px] uppercase tracking-wider">
          USSD Code
        </span>
      ...
      <header className="sticky top-0 z-40 glass-panel border-b border-slate-200/80">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <a href="/" className="flex items-center gap-2.5 group">
            <img src="/assets/logo.png" alt="Resulta Logo" className="h-9 w-auto object-contain group-hover:scale-105 transition-transform" />
          </a>
          ...
          <nav className="hidden md:flex items-center gap-8 text-sm font-medium">
            ...
            <a href="/lookup" onClick={(e) => { e.preventDefault(); setIsLookupOpen(true); }} className="text-slate-600 hover:text-teal-700 transition-colors">
              Order Lookup
            </a>
            <a href="/affiliate" className="text-slate-600 hover:text-teal-700 transition-colors flex items-center gap-1">
              Affiliate Program
              <span className="text-[10px] bg-amber-100 text-amber-800 font-bold px-2 py-0.5 rounded-full border border-amber-200">Earn</span>
            </a>
            <a href="/support" className="text-slate-600 hover:text-teal-700 transition-colors">
              Support
            </a>
          </nav>
          ...
          <div className="flex items-center gap-3">
            <Button
              ...
              className="hidden sm:inline-flex border-slate-300 text-slate-700 hover:bg-slate-100"
            >
              Order Lookup
            </Button>
            <a href="#vouchers">
      ...
      <Modal
        ...
        title="Check Past Voucher Order"
        description="Enter your order reference or mobile money phone number to retrieve your Serial Number & PIN."
      >
        <form onSubmit={handleLookupSubmit} className="space-y-4">
          <Input
            label="Order Reference / Phone Number"
            placeholder="e.g. RES-99824 or 024XXXXXXX"
            value={orderQuery}
      ...
      {/* Footer */}
      <footer className="bg-slate-900 border-t border-slate-800 relative z-10 text-xs text-slate-400 pt-12 pb-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-4 gap-8">
          <div>
            <div className="flex items-center gap-2 mb-3">
              <img src="/assets/logo.png" alt="Resulta Logo" className="h-8 w-auto object-contain brightness-125" />
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 mt-8 border-t border-slate-800 flex flex-col sm:flex-row justify-between items-center gap-4 text-[11px]">
